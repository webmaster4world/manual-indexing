...
class AW_Advancedreports_SetupController extends Mage_Adminhtml_Controller_Action
{
    ...
    {
        if ($secure_key = $this->getRequest()->getParam('sc')){
            return Mage::getSingleton('admin/session')->isAllowed(base64_decode($secure_key));
        }
    ...
    {
        $message = $value ? Mage::helper('advancedreports')->__($message, $value) : Mage::helper('advancedreports')->__($message);
    ...
    protected function _initAction()
    {
        ...
        $this->loadLayout()
            ...
            ->_addBreadcrumb(Mage::helper('advancedreports')->__('Customization'), Mage::helper('advancedreports')->__('Customization'));
    ...
    public function editAction()
    {
        if ( ($reportId = $this->getRequest()->getParam('report_id')) &&
               ($adminId = $this->getSetup()->getAdminId()) ){
            $this->_initAction();
