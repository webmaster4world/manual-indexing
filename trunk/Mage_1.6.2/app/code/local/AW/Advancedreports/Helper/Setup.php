...
class AW_Advancedreports_Helper_Setup extends Varien_Object
{
    ...
    /**
     * Retrives customized config data
     * 
     * @param string $path
     * @return string
     */
    public function getCustomConfig($path)
    {
        if ($reportId = $this->getReportId()){
            if (($value = Mage::getModel('advancedreports/option')->load3params(
    ...
    /**
     * Retrives preapred data with collection and some differned magic
     * 
    ...
    public function getPreparedData($collection = null)
    {
        if (!$this->getData('prepared_data')){
            ...
            foreach ($this->_getPredefinedKeys() as $dataKey => $dataPath){
                ...
                $data[$dataKey] = $value;
                $data[$dataKey.'_use_default'] = $is_default;
            }
    ...
    /**
     * Retrives rendfered Use Default Checkbox
    ...
    public function getCheckboxScopeHtml($field, $name, $checked = false)
    {
        ...
        $checkbox = '<input type="checkbox" id="'.$field->getId().'_use_default" class="product-option-scope-checkbox" name="'.$field->getId().'_use_default" onchange="chahged'.$field->getId().'();" value="1" '.$checkedHtml.'/>';
