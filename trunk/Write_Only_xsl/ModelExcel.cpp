...
#include <string.h>
  ...
  /* Excell cell convert to double or char */
  ExcelCell& ExcelCell::operator = (const QVariant v) {
      
      bool ok;
      qreal dtemp = v.toDouble(&ok);
      if ( dtemp !=0 &&  dtemp > 0 && ok || dtemp !=0 &&  dtemp < 0 && ok) {
          m_nType = TYPE_NUMBER;
          m_nValue = dtemp;
      } else {
          m_nType = TYPE_STRING;
          const QByteArray dat = v.toByteArray();
          if (m_pchValue != NULL) {
          /////free (m_pchValue);
              m_pchValue = NULL;
          }
          m_pchValue = strdup(dat.constData());
      }
      return *this;
  }
