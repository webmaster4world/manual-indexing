                            ...
                            const QString &user, const QString &passwd, int port)
{
    ...
    runningdriver = driver.toLower();
    
    qDebug() << "### runningdriver  "  << runningdriver;
...
{
    QString cfileName = QFileDialog::getSaveFileName(this, "Export CSV text",QString(setter.value("lastdirDir").toString()), "*.csv");
...
{
    QString cfileName = QFileDialog::getSaveFileName(this, "Export SQLITE3 UTF-8 text",QString(setter.value("lastdirDir").toString()), "*.sql");
    bool ok;
...
void Browser::ExportXmlPrimary( QString qx )
{
              ...
              QString xfileName = QFileDialog::getSaveFileName(this, "Export XML",QString(setter.value("lastdirDir").toString()), "*.xml");
              ...
              QString dirsave = xfileName.left(xfileName.lastIndexOf("/"))+"/";
              setter.setValue("lastdirDir",dirsave);
...
QString Browser::SqlCreateTable( const QString tablein )
{
   QString stru;
    QString tablex = tablein;
    const QString takia = QString("`%1`").arg(tablex.toLower());
    const QString takiunix = QString("`%1`").arg(tablex.toUpper());
    
    const QString infost = QString("SHOW CREATE TABLE %1").arg(tablein);
    QSqlQuery query(infost,connectionWidget->currentDatabase());
       QSqlRecord rec = query.record();
            while (query.next()) { 
                 stru = query.value(1).toString();
                Q_ASSERT(stru.size() > 1);
                 if (OSWinServer()) {  /* uppercase on window =! */
                 stru = stru.replace(takia,takiunix);
                 }
                 stru.append(";\n");
            } 
   return stru;
...
/* discovery if window mysql server uppercase fieldname go to lowercase!!!! */
bool Browser::OSWinServer()
{
    
    bool winos;  /* 0/1 */
    QString words;
    QSqlQuery query("SHOW GLOBAL VARIABLES",connectionWidget->currentDatabase());
       QSqlRecord rec = query.record();
        int sumcol = rec.count();
            while (query.next()) {
                    for(int i=0;i<sumcol;i++){
                      QString value = query.value(1).toString()+" ";
                      words.append(value.toLower());
                    }
            } 
       if (words.contains("-linux-")) {
        winos = false; 
       } else {
        winos = true;    
       }
